...
const Player: React.FC<PlayerProps> = React.memo(({
  ...
  onSetSleepTimer,
  stopAfterChapter,
  onSetStopAfterChapter,
  useBookSettings,
  onSetUseBookSettings,
  onNext,
  onPrev,
  onSeek,
  playbackCurrentTime: playbackCurrentTimeProp,
  playbackDuration,
  isFetching,
  onSeekToTime,
  autoplayBlocked,
  ...
  const { audioCurrentTime: tickCurrentTime } = usePlaybackTick();
  const playbackCurrentTime = playbackCurrentTimeProp ?? tickCurrentTime;
  ...
  const calcValueFromEvent = (e: React.PointerEvent<HTMLDivElement>, logTag?: string) => {
    ...
    if (debugMode && logTag) {
      ...
      const targetOffset =
        scrubMode === "offset" ? Math.round(ratio * totalLengthChars) : undefined;
  ...
  return (
    <div
      className={`border-t transition-all duration-300 relative z-20 bg-surface text-theme border-theme ui-font`}
    >
      ...
      {autoplayBlocked && (
        <div className="absolute inset-0 z-50 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center animate-in fade-in duration-300">
          <button onClick={onPlay} className="flex flex-col items-center gap-4 group">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-4 px-4 lg:px-8 pt-5 select-none">
          <span className="text-[11px] font-black font-mono opacity-60 min-w-[40px] text-left">
            {displayTime}
          </span>
          <div
          ...
          >
            <div
              className={`h-1.5 rounded-full ${accentBg} transition-all duration-75 shadow-sm pointer-events-none`}
          ...
          <span className="text-[11px] font-black font-mono opacity-60 min-w-[40px] text-right">
            {displayTotal}
          </span>
        </div>
        ...
        <div className="px-4 lg:px-8 py-2 lg:py-6 flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between w-full">
            ...
            >
              <SettingsIcon
                className={`w-6 h-6 ${isExpandedMobile ? "text-[color:var(--tvx-accent)]" : ""}`}
              />
            </button>

            {/* Centered Controls */}
            <div
              className={`flex items-center ${isMobile ? "gap-2" : "gap-4 lg:gap-12"} flex-1 justify-center`}
            >
              <button onClick={onPrev} className="p-3 hover:scale-110 transition-transform">
                <SkipBack className="w-7 h-7 lg:w-8 lg:h-8" />
              </button>
              ...
              >
                {isFetching ? (
                  ...
                  <>
                    <Play
                      className={`w-8 h-8 lg:w-10 lg:h-10 fill-current ${isPlaying ? "hidden" : "block ml-1"}`}
                    />
                    <Pause
                      className={`w-8 h-8 lg:w-10 lg:h-10 fill-current ${isPlaying ? "block" : "hidden"}`}
                    />
                  </>
                )}
              </button>
              ...
              <button onClick={onNext} className="p-3 hover:scale-110 transition-transform">
                <SkipForward className="w-7 h-7 lg:w-8 lg:h-8" />
              </button>
            </div>
            ...
            <div className="hidden lg:flex items-center gap-4 relative">
              <button
                ...
                className={`flex items-center gap-2 px-5 py-3 rounded-full border border-theme text-xs font-black shadow-sm transition-all ${sleepTimer || stopAfterChapter ? "bg-[color:var(--tvx-accent)] text-white" : "opacity-60"}`}
              >
                <Clock className="w-4 h-4" />{" "}
                {sleepTimer ? formatTime(sleepTimer / speed) : "Sleep"}
              </button>
              {showSleepMenu && (
                <div className="absolute bottom-full mb-3 right-0 w-56 rounded-2xl border border-theme shadow-2xl p-2 z-[60] bg-surface">
                  {[15, 30, 60].map((m) => (
                    <button
                      ...
                      className="w-full text-left px-3 py-3 text-[13px] font-bold rounded-xl hover:bg-black/5"
                    >
                      {m} Minutes
                    </button>
                  ))}
                  ...
                  <button
                    onClick={() => {
                      onSetStopAfterChapter(!stopAfterChapter);
                      setShowSleepMenu(false);
                    }}
                    className={`w-full text-left px-3 py-3 text-[13px] font-black rounded-xl flex items-center justify-between ${stopAfterChapter ? "text-[color:var(--tvx-accent)]" : ""}`}
                  >
                    Stop after Chapter <Repeat className="w-3.5 h-3.5" />
                  </button>
                </div>
              )}
            </div>
          ...
          <div
            className={`${isExpandedMobile ? "flex" : "hidden"} lg:flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-12 py-4 lg:py-2 border-t border-theme lg:border-t-0 mt-2 lg:mt-0 animate-in slide-in-from-bottom-2`}
          >
            <div className="flex flex-col lg:flex-row items-center gap-6 lg:gap-12">
              <div className="flex flex-col gap-1.5 items-center lg:items-start">
                <span className="text-[10px] font-black uppercase tracking-widest opacity-60 ml-0.5">
                  Playback Speed
                </span>
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="h-1.5 w-32 accent-indigo-600"
                  />
            ...
            <div className="lg:hidden flex flex-col items-center gap-3 w-full border-t border-black/5 pt-4">
              ...
              <div className="flex gap-2 w-full max-w-xs">
                {[15, 30].map((m) => (
                  <button
                    key={m}
                    onClick={() => onSetSleepTimer(m * 60)}
                    className={`flex-1 py-3 rounded-xl border text-[10px] font-black ${sleepTimer === m * 60 ? "bg-indigo-600 text-white" : "opacity-60"}`}
                  >
                    {m}m
                  </button>
                ))}
                <button
                  onClick={() => onSetStopAfterChapter(!stopAfterChapter)}
                  className={`flex-1 py-3 rounded-xl border text-[10px] font-black ${stopAfterChapter ? "bg-indigo-600 text-white" : "opacity-60"}`}
                >
                  CH Stop
                </button>
