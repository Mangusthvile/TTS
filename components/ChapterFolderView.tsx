  ...
  const renderRow = (c: any) => {
    ...
    return (
      <div
        ...
        className={`grid grid-cols-[86px_1fr_120px_100px_64px] items-center px-6 py-4 cursor-pointer select-none border-b last:border-0 transition-colors ${isDark ? 'hover:bg-white/5 border-slate-800' : 'hover:bg-black/5 border-black/5'} ${c.isCompleted ? 'opacity-60' : ''}`}
      >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-8 pb-12 pt-8">
        <div className={`rounded-[2.5rem] border shadow-sm overflow-hidden ${cardBg}`}>
          {chapters.length === 0 ? (
            ...
            <div>
              <div className={`grid grid-cols-[86px_1fr_120px_100px_64px] px-6 py-4 text-[11px] font-black uppercase tracking-widest border-b ${isDark ? 'border-slate-800 bg-slate-950/40 text-indigo-400' : 'border-black/5 bg-black/5 text-indigo-600'}`}>
                <div>Index</div>
            ...
            <div className={`divide-y ${isDark ? 'divide-slate-800' : 'divide-black/5'}`}>
              {chapters.map((c) => (
                <button
                  key={c.id}
                  onClick={() => onOpenChapter(c.id)}
                  className={`w-full text-left px-6 py-4 flex items-center gap-4 transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-black/5'} ${c.isCompleted ? 'opacity-60' : ''}`}
                >
