...
const ReaderToolbar: React.FC<Props> = ({
  ...
  return (
    <div className="flex items-center gap-1 relative">
      ...
      {showToolsMenu && (
        <div
          className={`absolute right-0 top-12 z-20 min-w-[180px] rounded-2xl shadow-2xl p-2 ${theme === Theme.DARK ? "bg-slate-900 border border-white/10" : theme === Theme.SEPIA ? "bg-[#efe6d5] border border-black/10" : "bg-white border border-black/10"}`}
        >
          {onAddChapter && (
            <button
              onClick={() => {
