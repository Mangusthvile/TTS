...
export function useChapterPaging(setState: React.Dispatch<React.SetStateAction<AppState>>) {
  const [chapterPagingByBook, setChapterPagingByBook] = useState<
    Record<string, { afterIndex: number; hasMore: boolean; loading: boolean }>
  >({});
  const chapterPagingRef = useRef<
    Record<string, { afterIndex: number; hasMore: boolean; loading: boolean }>
  >({});
  const warnedTextClearRef = useRef<Set<string>>(new Set());

  useEffect(() => {
    chapterPagingRef.current = chapterPagingByBook;
  }, [chapterPagingByBook]);

  const preserveChapterContent = useCallback(
    (prev: Chapter, next: Chapter, reason: string): Chapter => {
      const prevContent =
        typeof prev.content === "string" && prev.content.length > 0 ? prev.content : null;
      const nextContent =
        typeof next.content === "string" && next.content.length > 0 ? next.content : null;
  ...
  const loadMoreChapters = useCallback(
    async (bookId: string, reset: boolean = false) => {
      const limit = 200;

      const current = chapterPagingRef.current[bookId] ?? {
        afterIndex: -1,
        hasMore: true,
        loading: false,
      };
      ...
      setChapterPagingByBook((p) => ({
        ...
        [bookId]: {
          ...current,
          afterIndex: reset ? -1 : current.afterIndex,
          hasMore: true,
          loading: true,
        },
      }));
