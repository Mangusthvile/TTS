...
/** Throttle (ms) for tickStore tick updates on mobile to reduce consumer re-renders. */
const SYNC_UI_THROTTLE_MS = 300;

/** Throttle (ms) for progress tick commits on mobile; reduces localStorage write frequency. */
const PROGRESS_TICK_THROTTLE_MOBILE_MS = 1000;
...
export function usePlayback(args: {
  ...
  pendingCueFallbackRef: React.MutableRefObject<{
    chapterId: string;
    text: string;
    prefixLen: number;
  } | null>;
  activeSpeakTextRef: React.MutableRefObject<{
    chapterId: string;
    text: string;
    prefixLen: number;
  } | null>;
  cueIntegrityRef: React.MutableRefObject<{
    chapterId: string;
    driftCount: number;
    lastRebuildAt: number;
    lastNoticeAt: number;
  }>;
  cueDurationRef: React.MutableRefObject<{
    chapterId: string;
  ...
  const lastSyncUiUpdateRef = useRef(0);
  ...
  const nextChapterPreloadRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (playbackPhase === "SEEKING") {
      ...
      const timer = setTimeout(() => {
        const stillInFlight = seekTxnRef.current.inFlight;
        trace("watchdog:seek_timeout", { inFlight: stillInFlight });
  ...
  useEffect(() => {
    ...
    const unsubscribe = playbackAdapter.onState((playbackState) => {
      ...
      if (!playbackState.isPlaying) {
        setPlaybackPhase((p) => (p === "PLAYING_INTRO" || p === "PLAYING_BODY" ? "READY" : p));
      }
    });
    return () => {
  ...
  const commitProgressUpdate: CommitProgressFn = useCallback(
    (
      ...
      const now = Date.now();
      const throttleMs = effectiveMobileMode ? PROGRESS_TICK_THROTTLE_MOBILE_MS : 250;
      ...
      if (book.currentChapterId && book.currentChapterId !== chapterId) {
        if (now - lastProgressWarnRef.current > 2000) {
          lastProgressWarnRef.current = now;
          console.warn("[Progress] skip commit for non-active chapter", {
            reason,
            bookId,
            chapterId,
      ...
      const isCompletionReason =
        effectiveReason === "ended" ||
        effectiveReason === "scrubToEnd" ||
        effectiveReason === "seekToNearEnd";
      ...
      if (cIdx === -1) {
        if (now - lastProgressWarnRef.current > 2000) {
          lastProgressWarnRef.current = now;
          console.warn("[Progress] skipped commit; chapter not found", {
            reason,
            bookId,
            chapterId,
          });
      ...
      const updated: Chapter = {
        ...
        progressChars:
          typeof next.progressChars === "number" ? next.progressChars : chapter.progressChars,
      ...
      if (
        ...
        const shouldPersistProgress = !["scrub", "scrubToEnd", "seek", "seekToNearEnd"].includes(
          effectiveReason
        );
        if (shouldPersistProgress) {
          try {
            ...
            const nextEntry: ProgressStoreEntry = {
            ...
            if (!isSame) {
              ...
              // Tick commits use persistToNative false (localStorage only); SQLite updated on pause/ended/reset and periodic flush.
              const persistToNative =
                effectiveReason === "pause" ||
                effectiveReason === "ended" ||
                effectiveReason === "reset";
              writeProgressStore({ ...store, books }, { persistToNative });
              if (persistToNative) {
                void upsertSingleChapterProgress(bookId, chapterId, nextEntry, {
                  forceReset: effectiveReason === "reset",
                });
              }
            }
          } catch (e) {
        ...
        if (effectiveReason === "reset") {
          const s = stateRef.current;
          const b = s.books.find((bk) => bk.id === bookId);
          if (b?.currentChapterId === chapterId) speechController.safeStop();
        }
  ...
  const handleSyncUpdate = useCallback(
    (meta: PlaybackMetadata & { completed?: boolean }) => {
      ...
      const now = Date.now();
      const allowSyncUi = !effectiveMobileMode || (now - lastSyncUiUpdateRef.current >= SYNC_UI_THROTTLE_MS);
      ...
      if (
        ["LOADING_AUDIO", "SEEKING", "TRANSITIONING", "LOADING_TEXT", "SCRUBBING"].includes(
          playbackPhase
        )
      ) {
        if (playbackPhase === "SEEKING") {
          if (allowSyncUi) {
            ...
            if (b && b.currentChapterId) {
              tickStore.setPlaybackSnapshot({ chapterId: b.currentChapterId, percent });
            }
            lastSnapshotRef.current = now;
            lastSyncUiUpdateRef.current = now;
          }
          if (allowSyncUi && meta.currentTime > 0) {
            tickStore.setAudioCurrentTime(meta.currentTime);
          }
        }
        return;
      }
      ...
      if (meta.currentTime > 0 || meta.duration > 0) {
        ...
        if (s.activeBookId && s.books) {
          ...
          if (b && chapterId) {
            if (allowSyncUi && now - lastSnapshotRef.current > 100) {
              const percent = meta.duration > 0 ? meta.currentTime / meta.duration : 0;
              tickStore.setPlaybackSnapshot({ chapterId, percent });
              lastSnapshotRef.current = now;
              lastSyncUiUpdateRef.current = now;
            }
            if (
              playbackPhase === "PLAYING_BODY" &&
              meta.duration > 60 &&
              meta.duration - meta.currentTime <= 120 &&
              nextChapterPreloadRef.current !== chapterId
            ) {
              const sorted = normalizeChapterOrder(b.chapters || []);
              const idx = sorted.findIndex((c) => c.id === chapterId);
              const next = idx >= 0 && idx < sorted.length - 1 ? sorted[idx + 1] : null;
              if (next) {
                nextChapterPreloadRef.current = chapterId;
                void ensureChapterContentLoaded(b.id, next.id, chapterSessionRef.current);
              }
            }
            ...
            let reason: ProgressCommitReason = meta.completed ? "ended" : "tick";
            let allowDecrease = false;
            if (
              chapter &&
              reason === "tick" &&
              typeof chapter.progressSec === "number" &&
              Number.isFinite(meta.currentTime)
            ) {
              if (meta.currentTime < chapter.progressSec - 0.5) {
                reason = "seek";
                allowDecrease = true;
              }
            }
            commitProgressUpdate(b.id, chapterId, meta, reason, false, false, allowDecrease);
          }
        }
      }
      if (onSyncMeta) {
  ...
  useEffect(() => {
    ...
    if (typeof initialSpeed === "number" && initialSpeed > 0 && initialSpeed !== 1) {
  ...
  const confirmSeekLanding = useCallback(
    async (targetMs: number, seekId: number) => {
      const adapter = playbackAdapter ?? speechController.getPlaybackAdapter();
      if (!adapter) return false;
      // Give native layer time to apply seek and emit state (avoids false "not landed" on first poll)
      await waitForMs(100);
      const POLL_ITERATIONS = 15;
      const POLL_INTERVAL_MS = 100;
      const TOLERANCE_MS = 350;
      for (let i = 0; i < POLL_ITERATIONS; i += 1) {
        ...
        if (Math.abs(positionMs - targetMs) <= TOLERANCE_MS) {
          trace("seek:landed", { targetMs, positionMs, attempt: i + 1 });
          return true;
        }
        await waitForMs(POLL_INTERVAL_MS);
      }
      trace("seek:confirm_timeout", { targetMs, lastPosition: adapter.getState().positionMs });
      return false;
    },
    [playbackAdapter]
  );

  const performSeekToMs = useCallback(
    async (targetMs: number, reason: string) => {
      ...
      updatePhase("SEEKING");
      trace("seek:start", { targetMs, reason, seekId });
      ...
      if (
        !landed &&
        seekTxnRef.current.id === seekId &&
        !(adapter instanceof DesktopPlaybackAdapter)
      ) {
        trace("seek:retry1", { targetMs });
        try {
          await adapter.play();
        } catch {}
        await waitForMs(50);
      ...
      if (!landed) {
        pushNotice({
          message: "Seek didn't complete. Position may have changed.",
          type: "info",
          ms: 2000,
        });
      }
    ...
    [confirmSeekLanding, playbackAdapter, updatePhase, pushNotice]
  );

  const resumePlaybackAfterSeek = useCallback(
  ...
  const handleSeekOffsetCommit = useCallback(
    async (offset: number) => {
      isScrubbingRef.current = false;
      setIsScrubbing(false);
      const wasPlaying = resumeAfterScrubRef.current;
  ...
  const handleScrubEndOffset = useCallback(
    (offset: number) => {
  ...
  const loadChapterSession = useCallback(
    async (targetChapterId: string, reason: "user" | "auto", attempt = 1) => {
      ...
      const cached = lruMapGet(chapterTextCacheRef.current, chapterCacheKey);
      const stateContent =
      ...
      if (!effectiveContent) {
        const chapterLabel = (chapter.title || "").trim() || `Chapter ${chapter.index}`;
        if (reason === "auto") {
          if (attempt === 1) {
            pushNotice({ message: "Retrying next chapter…", type: "info", ms: 2000 });
            ...
            setIsLoadingChapter(false);
            loadChapterSession(targetChapterId, "auto", 2);
            return;
          }
          pushNotice({
            message: `Autoplay stopped: missing chapter text for "${chapterLabel}". Run Fix Integrity or re-import.`,
            type: "error",
            ms: 6000,
          });
          setIsPlaying(false);
          setAutoplayBlocked(false);
          tickStore.setPlaybackSnapshot(null);
          updatePhase("READY");
          setIsLoadingChapter(false);
          return;
        }
        pushNotice({
          message: "Unable to load chapter text. Keeping previous content.",
          type: "info",
          ms: 4000,
        });
        setPlaybackPhase("READY");
      ...
      try {
        ...
        if (!paragraphMap || !paragraphMap.paragraphs || paragraphMap.paragraphs.length === 0) {
          ...
          const textToSpeakForPara = textToSpeak;
          const chapterIdForPara = chapter.id;
          const runDeferred = async () => {
            try {
              const built = buildParagraphMap(textToSpeakForPara, chapterIdForPara);
              await saveParagraphMap(chapterIdForPara, built);
              if (chapterSessionRef.current === session) {
                setActiveParagraphMap(built);
                console.log("[Highlight] paragraph map generated (deferred)", {
                  chapterId: chapterIdForPara,
                  paragraphCount: built.paragraphs.length,
                });
              }
            } catch (e) {
              console.warn("[Highlight] deferred paragraph map failed", e);
            }
          };
          if (typeof requestIdleCallback !== "undefined") {
            requestIdleCallback(() => void runDeferred(), { timeout: 3000 });
          } else {
      ...
      if (isNativePlatform) {
        ...
        const localPlaybackUrl = shouldUseLocalAudio
          ? await resolveChapterAudioUrl(chapter.id, uiMode)
          : null;
      ...
      if (shouldUseLocalAudio) {
        ...
        const maxQueue = 20; // Pre-load more chapters for smoother long playback (100+ chapters)
  ...
  const handlePlaybackItemChanged = useCallback(
    (nextId: string | null, _prevId: string | null) => {
      if (!nextId) return;
  ...
  const handleNextChapter = useCallback(
    (autoTrigger = false) => {
      ...
      const normalizeVolumeName = (chapter: Chapter | undefined): string | null => {
        const name =
          typeof (chapter as any)?.volumeName === "string"
            ? String((chapter as any).volumeName).trim()
            : "";
        if (!name) return null;
      ...
      if (idx >= 0 && idx < sorted.length - 1) {
        if (autoTrigger) {
          ...
          pushNotice({
            message:
              currentVol && nextVol && currentVol !== nextVol
                ? `Moving onto ${nextVol} - ${base}`
                : base,
            type: "info",
          });
