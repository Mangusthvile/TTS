...
import {
  ...
  moveFileToTrash,
  openFolderPicker,
  listFilesSortedByModified,
  resolveFolderIdByName,
  listSaveFileCandidates,
  createDriveFolder,
  listFoldersInFolder,
  findTaleVoxRoots,
  getDriveFileParentIds,
} from "./services/driveService";
import {
  ensureChapterDriveStorageFolder,
  findChapterDriveStorageFolder,
  listVolumeFolders,
} from "./services/driveChapterFolders";
...
import {
  listBooks as libraryListBooks,
  upsertBook as libraryUpsertBook,
  deleteBook as libraryDeleteBook,
  listChaptersPage as libraryListChaptersPage,
  upsertChapterMeta as libraryUpsertChapterMeta,
  deleteChapter as libraryDeleteChapter,
  saveChapterText as librarySaveChapterText,
  loadChapterText as libraryLoadChapterText,
  getChaptersByIds as libraryGetChaptersByIds,
  ...
  listChapterTombstones as libraryListChapterTombstones,
  upsertChapterTombstone as libraryUpsertChapterTombstone,
} from "./services/libraryStore";
import { commitProgressLocal } from "./services/progressStore";
import { bootstrapCore, ensureAuthReady } from "./src/app/bootstrap";
import {
  X,
  Loader2,
  FolderSync,
...
import {
  enqueueGenerateAudio,
  enqueueCloudGenerateBookAudio,
  getJobById,
  getJobRunnerCapability,
  startBookGenerationJob,
} from "./services/jobRunnerService";
import {
  getNativeChapterTextCount,
  ensureNativeBook,
  ensureNativeChapter,
  ensureNativeChapterText,
  hasNativeBook,
} from "./services/nativeLibraryBridge";
...
import { lruMapGet, lruMapSet, CHAPTER_TEXT_REF_CACHE_MAX } from "./utils/lruCache";
import {
...
import {
  applyStartupProgressConflictChoice,
  readProgressStore,
  readProgressStoreForSave,
  writeProgressStore,
  flushProgressStoreToDurable,
...
import { runAllProgressFlushes } from "./utils/progressFlush";
...
import { useJobActions } from "./src/app/state/useJobActions";
import { useAttachments } from "./src/app/state/useAttachments";
...
import {
  applyFullSnapshot,
  buildFullSnapshot,
  loadProgressFileFromDrive,
  readLocalSnapshotMeta,
  restoreFromDriveIfAvailable,
  saveProgressFileToDrive,
  saveToDrive,
...
const RESERVED_DRIVE_BOOK_FOLDER_NAMES = new Set(["meta", "attachments", "trash", "text", "audio"]);
...
const buildVolumeOrderFromDriveSync = (
  ...
  const hasExplicitOrder =
    Array.isArray(existingOrder) && existingOrder.some((name) => typeof name === "string");

  if (Array.isArray(existingOrder)) {
    existingOrder.forEach((name) => pushVolumeName(next, seen, name));
  }

  // Only use Drive folder names to seed the order when there is
  // no explicit volumeOrder yet. Once the user has customized
  // volume ordering, we keep that order stable and ignore Drive
  // folder ordering so it can't overwrite user preferences.
  if (!hasExplicitOrder) {
...
const App: React.FC = () => {
  ...
  const { diagnosticsReport, refreshDiagnostics, handleSaveDiagnostics, jobRunnerAvailable } =
    useDiagnostics(pushNoticeSafe);

  const chapterSessionRef = useRef(0);
  const [chapterSessionForDiag, setChapterSessionForDiag] = useState(0);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    let backHandle: { remove: () => Promise<void> | void } | null = null;
    ...
    const register = async () => {
      try {
        ...
        // Force critical save on pause to prevent data loss if killed
        pauseHandle = await CapacitorApp.addListener("pause", async () => {
          // Refresh Drive token so native workers have a valid token when running in background
          ensureValidToken(false).catch(() => {});
          // Commit current player position to progress store (bypasses throttle)
          speechController.forceFlushProgressForBackground();
          // Persist active chapter progress only; avoid rewriting full-book progress snapshots.
          try {
            const activeBookId = stateRef.current.activeBookId;
            const activeBook = activeBookId
              ? stateRef.current.books.find((b) => b.id === activeBookId)
              : null;
            const playbackMeta = speechController.getMetadata();
            const chapterId = playbackMeta.chapterId ?? activeBook?.currentChapterId;
            if (activeBookId && activeBook && chapterId) {
              const chapter = activeBook.chapters.find((c) => c.id === chapterId);
              const playbackCompleted = (playbackMeta as any)?.completed === true;
              const durationSec =
                typeof playbackMeta.duration === "number" && playbackMeta.duration > 0
                  ? playbackMeta.duration
                  : chapter?.durationSec;
              const timeSecCandidate =
                typeof playbackMeta.currentTime === "number" &&
                Number.isFinite(playbackMeta.currentTime)
                  ? Math.max(0, playbackMeta.currentTime)
                  : (chapter?.progressSec ?? 0);
              const completed =
                playbackCompleted ||
                chapter?.isCompleted === true ||
                (typeof durationSec === "number" &&
                  durationSec > 0 &&
                  timeSecCandidate >= Math.max(0, durationSec - 0.5));
              await commitProgressLocal({
                bookId: activeBookId,
                chapterId,
                timeSec: timeSecCandidate,
                durationSec,
            ...
            console.warn("[App] Pause active-chapter save failed", e);
          }
          await flushProgressStoreToDurable({ immediate: true }).catch(() => {});
        });
      } catch (err) {
        if (!cancelled) {
          console.warn("[App] failed to register native listeners", err);
        }
      }
    };
    ...
    return () => {
      ...
      if (pauseHandle) void pauseHandle.remove();
    };
  }, []);

  // Single periodic flush: localStorage → SQLite (Phase 5). Also on pause/end/background via usePlayback and appStateChange.
  // 60s interval limits SQLite I/O on native during long sessions.
  useEffect(() => {
  ...
  // Before unload: force sync flush of progress so hard refresh doesn't lose data.
  useEffect(() => {
    if (typeof window === "undefined") return;
    const handleBeforeUnload = () => {
      runAllProgressFlushes();
    };
    window.addEventListener("beforeunload", handleBeforeUnload);
    return () => window.removeEventListener("beforeunload", handleBeforeUnload);
  }, []);

  // Phase 4.2: Persist last active book id for cold-start restore.
  // Progress commit + flush on background is handled in usePlayback (appStateChange).
  useEffect(() => {
    if (typeof window === "undefined" || !Capacitor.isNativePlatform()) return;
    const listener = CapacitorApp.addListener("appStateChange", ({ isActive }) => {
      if (!isActive) {
        const id = stateRef.current?.activeBookId;
        void Preferences.set({ key: LAST_ACTIVE_BOOK_ID_KEY, value: id ?? "" }).catch(() => {});
      }
    });
    return () => {
  ...
  const [state, setState] = useState<AppState>(() => {
    ...
    const mergedReaderSettings: ReaderSettings = {
      ...
      useCloudBatchForAudio:
        typeof parsed.readerSettings?.useCloudBatchForAudio === "boolean"
          ? parsed.readerSettings.useCloudBatchForAudio
          : defaultReaderSettings.useCloudBatchForAudio,
    };
    ...
    return {
      ...
      // Default off to reduce device temperature and battery use; user can enable in Settings.
  ...
  // Keep screen on when Settings > Keep Awake is enabled (like YouTube)
  useKeepAwake(state.keepAwake);
  ...
  const [activeTab, setActiveTab] = useState<
    "library" | "collection" | "reader" | "rules" | "settings"
  >("library");
  ...
  const collectionScrollByBookRef = useRef<Record<string, number>>({});
  const [collectionRestoreScrollTop, setCollectionRestoreScrollTop] = useState<number | null>(null);
  const [startupProgressConflict, setStartupProgressConflict] =
    useState<StartupProgressConflict | null>(null);
  const [applyingStartupProgressChoice, setApplyingStartupProgressChoice] = useState(false);
  ...
  const { chapterPagingByBook, setChapterPagingByBook, loadMoreChapters, preserveChapterContent } =
    useChapterPaging(setState);
  const [logJobs, setLogJobs] = useState<boolean>(
    () => localStorage.getItem(LOG_JOBS_KEY) === "true"
  );
  useEffect(() => {
    localStorage.setItem(LOG_JOBS_KEY, logJobs ? "true" : "false");
  }, [logJobs]);
  const {
    notificationStatus,
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (parsed?.bookId && typeof parsed.collectionScrollTop === "number") {
          collectionScrollByBookRef.current[parsed.bookId] = parsed.collectionScrollTop;
          setCollectionRestoreScrollTop(parsed.collectionScrollTop);
        }
  ...
  const pendingCueFallbackRef = useRef<{
    chapterId: string;
    text: string;
    prefixLen: number;
  } | null>(null);
  const activeSpeakTextRef = useRef<{ chapterId: string; text: string; prefixLen: number } | null>(
    null
  );
  const cueIntegrityRef = useRef<{
    chapterId: string;
    driftCount: number;
  ...
  useEffect(() => {
    activeCueMapRef.current = activeCueMap;
  }, [activeCueMap]);
  useEffect(() => {
    activeParagraphMapRef.current = activeParagraphMap;
  }, [activeParagraphMap]);
    ...
    try {
      ...
      try {
        const res = await Filesystem.readFile({
          path: `${appConfig.paths.audioDir}/${chapterId}.mp3`,
  ...
  const resolveLocalPathForUpload = useCallback(
    async (chapterId: string, fallbackPath?: string) => {
      const record = await getChapterAudioPath(chapterId);
  ...
  const uploadChapterNow = useCallback(
  ...
  const handleStartupProgressConflict = useCallback((conflict: StartupProgressConflict | null) => {
    setStartupProgressConflict(conflict);
  }, []);
  ...
  useEffect(() => {
    ...
    const onResume = async () => {
      try {
        // Refresh Drive token on resume so native workers get valid token for next batch
        ensureValidToken(false).catch(() => {});
        // Force reload progress from disk to catch background worker updates
        const stored = readProgressStore();
        if (stateRef.current.activeBookId) {
          const bookId = stateRef.current.activeBookId;
          const progressMap = stored.books[bookId] || {};
          // Force UI refresh with latest progress
          setState((prev) => {
            ...
            return {
              ...prev,
              books: prev.books.map((b) => (b.id === bookId ? { ...b, chapters: newChapters } : b)),
            };
          });
        }
        await refreshJobs();
        await refreshUploadQueueCount();
        pushNoticeSafe({ message: "App Resumed", type: "info", ms: 1000 });
      } catch (e) {
        console.warn("Resume refresh failed", e);
      }
    };
    const handle = CapacitorApp.addListener("resume", onResume);
    return () => {
      void handle.then((h) => h.remove());
    };
  }, [refreshJobs, refreshUploadQueueCount, pushNoticeSafe]);
  ...
  const {
    ...
    setIsDirty,
    navContextRef,
    onStartupProgressConflict: handleStartupProgressConflict,
    onStartupError: (e) =>
      authManager.getToken() ? "ready" : "signin",
  });
  ...
  // If we're stuck on the sign-in screen but auth is already valid (e.g. token loaded late),
  // advance to ready so Sync is available and we don't block the user.
  useEffect(() => {
    if (launchStage !== "signin") return;
    if (authState.status === "signed_in" && authManager.getToken()) {
      setLaunchStage("ready");
    }
  }, [launchStage, authState.status, setLaunchStage]);
  ...
  useEffect(() => {
    ...
    if (!bookId) return;
    const cached = collectionScrollByBookRef.current[bookId];
    if (typeof cached === "number" && Number.isFinite(cached)) {
      setCollectionRestoreScrollTop((prev) => (prev === cached ? prev : cached));
      return;
    }
    const fromNav =
      navContextRef.current?.bookId === bookId &&
      typeof navContextRef.current?.collectionScrollTop === "number" &&
      Number.isFinite(navContextRef.current.collectionScrollTop)
        ? navContextRef.current.collectionScrollTop
        : 0;
    collectionScrollByBookRef.current[bookId] = fromNav;
    setCollectionRestoreScrollTop((prev) => (prev === fromNav ? prev : fromNav));
  ...
  useEffect(() => {
    ...
    const refreshChapterMetaForJob = async (jobId: string, status: string, error?: any) => {
      try {
        ...
        if (job.type === "generateAudio" || job.type === "generate_book_audio") {
        ...
        if (job.type === "generateAudio" || job.type === "generate_book_audio") {
          ...
          const shouldFail = (c: Chapter | undefined) => {
            ...
            const ready = !!(
              c.cloudAudioFileId ||
              (c as any).audioDriveId ||
              c.audioStatus === AudioStatus.READY
            );
            return !ready && c.audioStatus !== AudioStatus.FAILED;
          };

          const toFail = ids.map((id) => byId.get(id)).filter((c) => shouldFail(c)) as Chapter[];
    ...
    const applyJobEvent = (event: any) => {
      ...
      if (nextStatus === "completed") {
        ...
        const completed = progress?.completed ?? total;
        const jobType = event.type ?? "job";
        const label =
          jobType === "drive_upload_queue" || jobType === "uploadQueue"
            ? "Upload"
            : jobType === "fixIntegrity"
              ? "Integrity fix"
              : "Audio generation";
        const detail =
          total > 0 ? ` – ${completed} chapter${completed === 1 ? "" : "s"} completed` : "";
        pushNotice({ message: `${label} complete${detail}`, type: "success", ms: 2500 });
      } else if (nextStatus === "failed") {
        const errMsg = String(event.error ?? progress?.error ?? "").trim() || "An error occurred";
        pushNotice({ message: `Job failed: ${errMsg}`, type: "error", ms: 0 });
      } else if (nextStatus === "canceled") {
        pushNotice({ message: "Job canceled", type: "info", ms: 2000 });
      }
    };
  ...
  const hasActiveCloudJobs = useMemo(() => {
    return jobs.some(
      (j) =>
        (j.status === "queued" || j.status === "running") &&
        j.type === "generate_book_audio" &&
        (j as any)?.payloadJson?.backend === "cloud"
    );
  }, [jobs]);

  useEffect(() => {
    if (!hasActiveJobs) return;
    const baseLocalMs = Capacitor.isNativePlatform?.() ? 2500 : 1500;
    const baseCloudMs = Capacitor.isNativePlatform?.()
      ? 7000
      : (appConfig.cloud?.jobPollIntervalMs ?? 5000);
    const intervalMs = hasActiveCloudJobs ? baseCloudMs : baseLocalMs;
  ...
  }, [hasActiveJobs, hasActiveCloudJobs, refreshJobs]);
  ...
  useEffect(() => {
    ...
    const DOWNLOADED_CHAPTER_CONCURRENCY = 8;
    const loadDownloads = async () => {
      const entries: DownloadedChapterInfo[] = [];
      const chapters = book.chapters;
      for (let i = 0; i < chapters.length; i += DOWNLOADED_CHAPTER_CONCURRENCY) {
        if (!mounted) return;
        const chunk = chapters.slice(i, i + DOWNLOADED_CHAPTER_CONCURRENCY);
        const records = await Promise.all(chunk.map((ch) => getChapterAudioPath(ch.id)));
  ...
  const {
    ...
    setAttachmentViewer,
    attachmentDownloads,
    editingAttachmentId: attachmentEditingId,
    editingFilename: attachmentEditingFilename,
    setEditingFilename: setAttachmentEditingFilename,
  ...
  }, [activeBook, isAddChapterOpen, isAuthorized]);
  ...
  const activeChapterKey = activeBook?.currentChapterId
    ? `${activeBook.id}:${activeBook.currentChapterId}`
    : null;
  const activeChapterText = useMemo(() => {
    const content = activeChapterMetadata?.content;
    if (typeof content === "string" && content.length > 0) return content;
    if (activeChapterKey) {
      return lruMapGet(chapterTextCacheRef.current, activeChapterKey) ?? "";
    }
    return "";
  ...
  useEffect(() => {
    ...
    const content =
      typeof activeChapterMetadata.content === "string" ? activeChapterMetadata.content : "";
    if (content.length > 0) {
      lruMapSet(chapterTextCacheRef.current, activeChapterKey, content, CHAPTER_TEXT_REF_CACHE_MAX);
      return;
    }
    const cached = lruMapGet(chapterTextCacheRef.current, activeChapterKey);
    if (cached && cached.length > 0) {
      trace("text:load", {
        chapterId: activeChapterMetadata.id,
        source: "memory",
        len: cached.length,
      });
      setState((p) => ({
  ...
  const persistReaderProgress = useCallback(
    (map: ProgressMap) => {
  ...
  const handleCollectionScroll = useCallback(
    (scrollTop: number) => {
      ...
      const bookId = stateRef.current.activeBookId;
      if (bookId) {
        collectionScrollByBookRef.current[bookId] = scrollTop;
      }
  ...
  const highlightSync = useHighlightSync({
    ...
    throttleMs: state.readerSettings.highlightUpdateRateMs ?? (Capacitor.isNativePlatform() ? 250 : 100),
  ...
  const cueMapLooksPrefixed = cuePrefixLen > 0 && cueMapMaxEnd > activeSpeechText.length + 2;
  ...
  const lastSyncAt =
    state.syncDiagnostics?.lastSyncSuccessAt ?? state.syncDiagnostics?.lastSyncAttemptAt;
  ...
  useEffect(() => {
    ...
    if (authState.status === "expired") {
      pushNotice({
        message: "Drive session expired. Reconnect required.",
        type: "reconnect",
        ms: 6000,
      });
    }
  ...
  const mergeProgressEntryIntoChapter = useCallback(
    (
      chapter: Chapter,
      progressEntry?: ProgressStoreEntry,
      options?: { forceProgress?: boolean; clearWhenMissing?: boolean }
    ): Chapter => {
      if (!progressEntry) {
      ...
      const entryDurationSec =
        typeof progressEntry.durationSec === "number" && Number.isFinite(progressEntry.durationSec)
          ? Math.max(0, progressEntry.durationSec)
          : undefined;

      let preferEntry = options?.forceProgress === true || entryUpdatedAt >= chapterUpdatedAt;
      if (
        !preferEntry &&
  ...
  const buildSnapshotState = useCallback(
    async (
      ...
      progressStorePayload: ProgressStorePayload
    ): Promise<AppState> => {
  ...
  const applyProgressStoreToLoadedBooks = useCallback(
    (progressStorePayload: ProgressStorePayload, options?: { clearWhenMissing?: boolean }) => {
      setState((prev) => ({
        ...prev,
        books: prev.books.map((book) => {
          ...
          const mergedChapters = orderChaptersForDisplay(
            (book.chapters ?? []).map((chapter) =>
              mergeProgressEntryIntoChapter(chapter, progressByChapter[chapter.id], {
                forceProgress: true,
                clearWhenMissing: options?.clearWhenMissing === true,
              })
            )
          );
          return {
    ...
    [mergeProgressEntryIntoChapter, setState]
  );

  const resolveStartupProgressConflict = useCallback(
    async (choice: "durable" | "local" = "durable") => {
      const conflict = startupProgressConflict;
      if (!conflict || applyingStartupProgressChoice) return;
      setApplyingStartupProgressChoice(true);
      try {
        const applied = await applyStartupProgressConflictChoice(choice);
        const payload = applied ?? (choice === "local" ? conflict.local : conflict.durable);
        applyProgressStoreToLoadedBooks(payload, { clearWhenMissing: true });
        setStartupProgressConflict(null);
        pushNoticeSafe({
          message:
            choice === "local"
              ? "Using local progress snapshot."
              : "Using durable progress snapshot.",
          type: "info",
          ms: 2500,
        });
      } catch (e) {
        console.warn("[Startup] progress conflict apply failed", e);
        setStartupProgressConflict(null);
        pushNoticeSafe({
          message: "Failed to apply selected progress source. Using durable snapshot.",
          type: "error",
          ms: 4500,
        });
        const fallback = await applyStartupProgressConflictChoice("durable").catch(() => null);
        if (fallback) {
          applyProgressStoreToLoadedBooks(fallback, { clearWhenMissing: true });
        } else {
          applyProgressStoreToLoadedBooks(conflict.durable, { clearWhenMissing: true });
        }
      } finally {
        setApplyingStartupProgressChoice(false);
      }
    },
    [
      applyingStartupProgressChoice,
      applyProgressStoreToLoadedBooks,
      pushNoticeSafe,
  ...
  useEffect(() => {
    if (launchStage !== "ready") return;
    if (startupProgressConflict) return;
    const pending = getPendingStartupProgressConflict();
    if (pending) {
      setStartupProgressConflict(pending);
    }
  }, [launchStage, startupProgressConflict]);

  useEffect(() => {
    if (launchStage !== "ready") return;
    if (!startupProgressConflict) return;
    if (applyingStartupProgressChoice) return;
    const timer = window.setTimeout(() => {
      void resolveStartupProgressConflict("durable");
    }, 12000);
    return () => window.clearTimeout(timer);
  }, [
    applyingStartupProgressChoice,
    launchStage,
    resolveStartupProgressConflict,
    startupProgressConflict,
  ]);
  ...
  const applyReaderProgressCommit = useCallback(
    (chapterId: string, next: ChapterProgress) => {
      const s = stateRef.current;
      ...
      setState((prev) => {
        ...
        updatedChapters[cIdx] = {
          ...
          durationSec:
            typeof next.durationSec === "number" ? next.durationSec : current.durationSec,
  ...
  const {
    ...
    handleChapterEnd,
    handleSkip,
    resetProgress: resetReaderProgress,
  } = useReaderProgress({
    chapters: activeBook
  ...
  const handleCueSyncUpdate = useCallback(
    (
      ...
      const chapterExists = !!(
        b &&
        meta.chapterId &&
        b.chapters.some((c) => c.id === meta.chapterId)
      );
      const playingChapterId = chapterExists
        ? (meta.chapterId ?? activeChapterId)
        : activeChapterId;
      ...
      if (cueMap && cueMap.cues.length > 0) {
        ...
        if (
          ...
          if (meta.duration > 0) {
            const linearOffset = Math.floor(
              (meta.currentTime / meta.duration) * activeSpeak.text.length
            );
        ...
        const nextOffset = Number.isFinite(meta.charOffset)
          ? meta.charOffset
          : stateRef.current.currentOffsetChars;
  ...
  const handleSaveState = useCallback(
    async (force = false, silent = false): Promise<boolean> => {
      ...
      if (!s.driveRootFolderId) {
        if (!silent) pushNotice({ message: "Link Drive to save to cloud", type: "info", ms: 3000 });
        return false;
      }
      ...
      const saveWithTimeout = async (): Promise<boolean> => {
        const timeoutMs = 120000; // 2 minutes timeout
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(
            () => reject(new Error("Save operation timed out after 2 minutes")),
            timeoutMs
          );
        });
        ...
        const savePromise = (async (): Promise<boolean> => {
          try {
            ...
            const progressStorePayload = await readProgressStoreForSave();
            const snapshotState = force
              ? await buildSnapshotState(stateRef.current, progressStorePayload)
              : s;
            ...
            const chapterTombstonesLists = await Promise.all(
              snapshotState.books.map((book) =>
                libraryListChapterTombstones(book.id).catch(() => [])
              )
            );
            const chapterTombstones = chapterTombstonesLists.flat();
            ...
            const snapshot = buildFullSnapshot({
              ...
              readerProgress,
              legacyProgressStore:
                (progressStorePayload as unknown as Record<string, unknown>) || {},
              chapterTombstones,
              attachments,
              jobs,
            ...
            const rootFolderId = snapshotState.driveRootFolderId || s.driveRootFolderId;
            if (!rootFolderId) {
              throw new Error("No Drive root folder ID available");
            }
            ...
            try {
              await saveProgressFileToDrive(
                saveRes.savesFolderId,
                readerProgress,
                progressStorePayload as unknown as Record<string, unknown>
              );
            } catch (e) {
              console.warn("[Save] Progress file upload failed", e);
            }
            ...
            setIsDirty(false);
            if (!silent) pushNotice({ message: "Saved to Drive", type: "success", ms: 2000 });
            return true;
          } catch (e: any) {
            throw e;
          }
      ...
      try {
        return await saveWithTimeout();
      } catch (e: any) {
        if (!silent)
          pushNotice({ message: "Save Failed: " + (e?.message || String(e)), type: "error" });
        updateDiagnostics({
  ...
  useEffect(() => {
    ...
    let intervalMinutes = Math.max(1, Number(state.autoSaveInterval) || 0);
    if (typeof Capacitor !== "undefined" && Capacitor.isNativePlatform?.()) {
      intervalMinutes = Math.min(120, intervalMinutes * 2);
    }
      ...
      const task = (async () => {
        ...
        const stateContent =
          typeof chapter.content === "string" && chapter.content.length > 0
            ...
            : null;
        if (stateContent) {
          lruMapSet(chapterTextCacheRef.current, key, stateContent, CHAPTER_TEXT_REF_CACHE_MAX);
          try {
        ...
        const memoryCached = lruMapGet(chapterTextCacheRef.current, key);
                    ...
                    chapters: b.chapters.map((c) =>
                      c.id === chapterId
                        ? {
                            ...
                            textLength: memoryCached.length,
                            updatedAt: Date.now(),
                          }
                        : c
                    ),
        ...
        try {
          ...
          if (typeof cached === "string" && cached.length > 0) {
            ...
            lruMapSet(chapterTextCacheRef.current, key, cached, CHAPTER_TEXT_REF_CACHE_MAX);
        ...
        if (computeMobileMode(s.readerSettings.uiMode)) {
          ...
          try {
            ...
            if (fileText && fileText.length > 0) {
              ...
              lruMapSet(chapterTextCacheRef.current, key, fileText, CHAPTER_TEXT_REF_CACHE_MAX);
              try {
        ...
        if (isAuthorized) {
          // Re-read from stateRef to pick up cloudTextFileId updates (e.g. from a job that just completed)
          const latestChapter =
            stateRef.current.books
              .find((b) => b.id === bookId)
              ?.chapters.find((c) => c.id === chapterId) ?? chapter;
          const textFileId = latestChapter.cloudTextFileId;
          let driveTextFetchFailed = false;

          if (textFileId) {
            trace("text:drive:load:start", { chapterId, fileId: textFileId, session });
            try {
              const text = await fetchDriveFile(textFileId);
              if (typeof text === "string") {
                ...
                lruMapSet(chapterTextCacheRef.current, key, text, CHAPTER_TEXT_REF_CACHE_MAX);
                trace("text:load", { chapterId, source: "drive", len: text.length });
                return text;
              }
            } catch (e: any) {
              traceError("text:drive:load:failed", e);
              driveTextFetchFailed = true;
            }
          }

          // Search-by-name fallback: when cloudTextFileId is absent or the stored ID is stale/404
          const driveFolderId = stateRef.current.books.find((b) => b.id === bookId)?.driveFolderId;
          if (driveFolderId && (!textFileId || driveTextFetchFailed)) {
            try {
              const fmt =
                (latestChapter.contentFormat ?? chapter.contentFormat) === "markdown"
                  ? "markdown"
                  : "text";
              const textName =
                latestChapter.filename || chapter.filename || buildTextName(bookId, chapterId, fmt);
              const volumeFolderId = await findChapterDriveStorageFolder(
                driveFolderId,
                latestChapter
              );
              const searchFolders =
                volumeFolderId && volumeFolderId !== driveFolderId
                  ? [volumeFolderId, driveFolderId]
                  : [driveFolderId];
              const candidates = await Promise.all(
                searchFolders.map((fid) => findFileSync(textName, fid))
              );
              const foundId = candidates.find((id) => !!id) || null;
              if (foundId) {
                const text = await fetchDriveFile(foundId);
                if (typeof text === "string") {
                  if (chapterSessionRef.current !== session) {
                    trace("text:drive:load:aborted", { reason: "stale_session" });
                    return null;
                  ...
                  // Persist the newly-found cloudTextFileId to SQLite so future loads use it directly
                  try {
                    await libraryUpsertChapterMeta(bookId, {
                      ...latestChapter,
                      cloudTextFileId: foundId,
                      content: undefined,
                    });
                  ...
                  lruMapSet(chapterTextCacheRef.current, key, text, CHAPTER_TEXT_REF_CACHE_MAX);
                  trace("text:load", { chapterId, source: "drive:search", len: text.length });
                  return text;
                }
              }
            } catch (e: any) {
              traceError("text:drive:search:failed", e);
            }
          }

          if (textFileId || driveTextFetchFailed) {
            pushNotice({ message: "Failed to load chapter text from Drive", type: "error" });
          }
        }
  ...
  const hardRefreshForChapter = useCallback(
    async (bookId: string, chapterId: string) => {
      const s = stateRef.current;
      const book = s.books.find((b) => b.id === bookId);
      ...
      try {
        const textName =
          ...
          buildTextName(
            book.id,
            chapter.id,
            chapter.contentFormat === "markdown" ? "markdown" : "text"
          );
  ...
  const handleReconcileProgress = useCallback(() => {
    ...
    const newBooks = s.books.map((b) => {
      ...
      const newChapters = b.chapters.map((c) => {
        ...
        if (
          normalized.isCompleted !== c.isCompleted ||
          Math.abs(normalized.progress - c.progress) > 0.01
        ) {
          changedCount++;
          return normalized;
        }
  ...
  const {
    ...
    cueDurationRef,
    isInIntroRef,
    onSessionChange: setChapterSessionForDiag,
    onSyncMeta: handleCueSyncUpdate,
  });

  const highlightDebugData = useMemo(
    () => ({
      // Intentionally omit tick from deps: App does not subscribe to tickStore, so App
      // does not re-render on every playback tick. Consumers that need live position
      // (e.g. PlaybackDiagnosticsOverlay) use usePlaybackTick() instead.
      positionMs: Math.floor(tickStore.getSnapshot().audioCurrentTime * 1000),
  ...
  }, [launchStage, getEffectivePlaybackSpeed]);
  ...
  const handleSmartOpenChapter = (id: string) => {
    ...
    if (clickedChapter.isCompleted) {
      ...
      if (nextIncomplete) {
        pushNotice({
          message: `Skipping completed ch.${clickedChapter.index} → ch.${nextIncomplete.index}`,
          type: "info",
        });
  ...
  useEffect(() => {
    ...
    const resolvedView =
      lastView === "settings" ? (pending.lastNonReaderViewType ?? "library") : lastView;
  ...
  const handleUpdateBookMeta = useCallback(
    async (book: Book) => {
      const s = stateRef.current;
  ...
  const handleUpdateChapter = useCallback(
    async (chapter: Chapter) => {
      ...
      const merged = existing
        ? preserveChapterContent(existing, { ...existing, ...chapter }, "handleUpdateChapter")
        : chapter;
      setState((prev) => ({
        ...prev,
        books: prev.books.map((b) =>
      ...
      // isCompleted is NOT saved by libraryUpsertChapterMeta (not in the chapters schema).
      // It lives exclusively in the progressStore. Write it there whenever it changes so
      // progress survives a hard close and reopen.
      const incomingIsCompleted = (chapter as any).isCompleted;
      const existingIsCompleted = (existing as any)?.isCompleted;
      if (typeof incomingIsCompleted === "boolean" && incomingIsCompleted !== existingIsCompleted) {
        void commitProgressLocal({
          bookId: book.id,
          chapterId: merged.id,
          timeSec: incomingIsCompleted ? ((merged as any).durationSec ?? 0) : 0,
          durationSec: (merged as any).durationSec,
          isComplete: incomingIsCompleted,
  ...
  const handleDeleteChapter = useCallback(
    async (chapterId: string) => {
      ...
      const renumbered = renumberChaptersSequentially(remaining);
      const nextChapters = orderChaptersForDisplay(renumbered);
      const nextCount =
        typeof book.chapterCount === "number"
          ? Math.max(0, book.chapterCount - 1)
          : nextChapters.length;
      const prevChapters = book.chapters;
      const prevChapterCount = book.chapterCount;

      setState((p) => ({
        ...p,
        books: p.books.map((b) =>
          b.id === book.id ? { ...b, chapters: nextChapters, chapterCount: nextCount } : b
        ),
      }));
      markDirty();
      ...
      const revert = () => {
        setState((p) => ({
          ...p,
          books: p.books.map((b) =>
            b.id === book.id ? { ...b, chapters: prevChapters, chapterCount: prevChapterCount } : b
          ),
        }));
        markDirty();
      };
      ...
      try {
        await libraryBulkUpsertChapters(
          book.id,
          renumbered.map((c) => ({ chapter: { ...c, content: undefined }, content: null }))
        );
      } catch (e: any) {
        console.warn("[TaleVox][Library] renumber after delete failed", e);
        revert();
        return;
      }
  ...
  const handleDeleteBookMeta = useCallback(
    async (id: string) => {
      try {
  ...
  const {
    ...
    handleShowWorkInfo,
    handleClearJobs,
  } = useJobActions({ stateRef, refreshJobs, setJobs, pushNotice });
  ...
  const handleSyncLibraryToNativeDb = useCallback(
    async (opts?: { bookId?: string; chapterIds?: string[] }) => {
      const runId = ++nativeSyncRunRef.current;
      const isCancelled = () => nativeSyncRunRef.current !== runId;
      const s = stateRef.current;
      const chapterFilter =
        opts?.chapterIds && opts.chapterIds.length ? new Set(opts.chapterIds) : null;
      let books = 0;
      let chapters = 0;
      let texts = 0;
      let failures = 0;
      ...
      const targetBooks = opts?.bookId ? s.books.filter((b) => b.id === opts.bookId) : s.books;
  ...
  const handleAddBook = async (
    ...
    if (backend === StorageBackend.DRIVE && !driveFolderId && state.driveRootFolderId) {
      try {
        ...
        pushNotice({ message: "Failed to create Drive folder", type: "error", ms: 0 });
        return;
      }
    }
  ...
  const handleChapterExtracted = async (data: any) => {
    ...
    const contentFormat: "text" | "markdown" =
      data.contentFormat === "markdown" ? "markdown" : "text";
    ...
    if (book.driveFolderId && isAuthorized) {
      try {
        const targetFolderId = await ensureChapterDriveStorageFolder(
          book.driveFolderId,
          newChapter
        );
        newChapter.cloudTextFileId = await uploadToDrive(
          targetFolderId,
          newChapter.filename,
          chapterContent
        );
    ...
    if (shouldAutoGenerateAudio) {
      ...
      void (async () => {
        try {
          ...
          pushNotice({
            message: `Auto audio generation failed: ${String(e?.message ?? e)}`,
            type: "error",
          });
        }
      })();
  ...
  const syncBookInventoryToDrive = async (book: Book): Promise<void> => {
    ...
    try {
      ...
      const root = {
        ...
        id: book.driveFolderId,
        name: book.driveFolderName ?? book.title,
      };
  ...
  const performFullDriveSync = async (manual = false) => {
    ...
    try {
      ...
      let updatedBooks = [...s.books];
      const [progressData, driveBooks] = await Promise.all([
        loadProgressFileFromDrive(savesId),
        listFoldersInFolder(booksId),
      ]);
      try {
        if (
          progressData &&
          (Object.keys(progressData.readerProgress || {}).length > 0 ||
            Object.keys(progressData.legacyProgressStore || {}).length > 0)
        ) {
          const localLastUpdated = getLocalProgressLastUpdated();
          if (progressData.savedAt > localLastUpdated) {
            if (Object.keys(progressData.readerProgress || {}).length > 0) {
              safeSetLocalStorage(
                "talevox_reader_progress",
                JSON.stringify(progressData.readerProgress)
              );
            }
            const normalized = normalizeProgressStore(
              progressData.legacyProgressStore ?? progressData.progressStore
            );
            if (normalized && Object.keys(normalized.books ?? {}).length > 0) {
          ...
          updatedBooks = updatedBooks.map((book) => {
            const progressMap = store.books?.[book.id] || {};
            const chaptersWithProgress = (book.chapters || []).map((c) => {
              const p = progressMap[c.id];
              if (!p) return c;
              const progressChars =
                p.percent != null && c.textLength
            ...
            return { ...book, chapters: chaptersWithProgress };
          });
        }
      } catch (e) {
        console.warn("[Sync] Progress file load failed", e);
      }
      let bookCounter = 0;
      for (const db of driveBooks) {
        ...
        let inventoryVolumeOrder: string[] | undefined;
        try {
          ...
          if (metaFolder) {
            ...
            if (invFile) {
              ...
              inventoryVolumeOrder = Array.isArray(inventory?.volumeOrder)
                ? inventory.volumeOrder
                : undefined;
              if (Array.isArray(inventory?.chapters)) {
                inventoryById = new Map(
                  inventory.chapters
                    .map((c: any) => {
                      const chapterId = c.chapterId ?? c.id ?? c.chapterID ?? c.chapter_id ?? null;
                      ...
                      const rawIdx =
                        ...
                        c.legacyIdx ??
                        null;
                      const parsedIdx = typeof rawIdx === "string" ? parseInt(rawIdx, 10) : rawIdx;
                      const idx =
                        Number.isFinite(parsedIdx) && parsedIdx > 0 ? Number(parsedIdx) : null;
                      const rawTitle =
                        c.title ?? c.name ?? c.chapterTitle ?? c.legacy?.title ?? null;
                      const title =
                        typeof rawTitle === "string" && rawTitle.trim().length
                          ? normalizeChapterTitle(rawTitle.trim())
                          : null;

                      const rawVolumeName = c.volumeName ?? c.volume ?? c.book ?? c.group ?? null;
                      const volumeName =
        ...
        if (!inventoryLoaded && inventoryById.size === 0 && existingMetaById.size > 0) {
          inventoryById = new Map(
            Array.from(existingMetaById.values()).map((c) => [
              c.id,
              {
                ...
                title:
                  typeof c.title === "string" && c.title.trim().length
                    ? normalizeChapterTitle(c.title.trim())
                    : null,
                volumeName:
                  typeof (c as any).volumeName === "string" && (c as any).volumeName.trim().length
                    ...
                    : null,
                volumeLocalChapter:
                  Number.isFinite((c as any).volumeLocalChapter) &&
                  Number((c as any).volumeLocalChapter) > 0
          ...
          if (manual && metaFolderId) {
            try {
              const bookIdForInv =
                existingBookIdx !== -1 ? updatedBooks[existingBookIdx].id : db.id;
              const existingBook = updatedBooks[existingBookIdx];
              const inventory: InventoryManifest = {
                schemaVersion: "3.0",
                bookId: bookIdForInv,
                ...
                chapters: Array.from(existingMetaById.values()).map((c) => ({
                  ...
                  textName:
                    ...
                    buildTextName(
                      bookIdForInv,
                      c.id,
                      c.contentFormat === "markdown" ? "markdown" : "text"
                    ),
                  audioName: buildMp3Name(bookIdForInv, c.id),
                })),
                volumeOrder: Array.isArray(existingBook.settings?.volumeOrder)
        ...
        for (const f of chapterScanFiles) {
          ...
          if (match) {
            ...
            if (!chaptersMap.has(id)) {
              ...
              const existingIdx =
                existing?.index != null && existing.index > 0 ? existing.index : null;
              const fallbackIdx = fallbackOrder.get(id) ?? 0;
              const invTitle =
                invMeta?.title && invMeta.title !== "Imported Chapter" ? invMeta.title : null;
              const title = normalizeChapterTitle(
                invTitle ?? existing?.title ?? invMeta?.title ?? "Imported Chapter"
              );
              ...
              const base: Partial<Chapter> = {
                ...
                volumeName:
                  (invMeta as any)?.volumeName ?? (existing as any)?.volumeName ?? hintedVolumeName,
                volumeLocalChapter:
        ...
        const driveChapters: Chapter[] = Array.from(chaptersMap.values())
          .filter(
            (c) => (c.cloudTextFileId || c.cloudAudioFileId) && !tombstoneIds.has(String(c.id))
          )
          .map((c) => {
        ...
        try {
          ...
          const attachmentsFolder = attachmentFolders.find(
            (f) => (f.name || "").toLowerCase() === "attachments"
          );
        ...
        if (existingBookIdx !== -1) {
          ...
          for (const dc of driveChapters) {
            const existingChIdx = mergedChapters.findIndex(
              (ec) => ec.id === dc.id || ec.cloudTextFileId === dc.cloudTextFileId
            );
            ...
            if (existingChIdx !== -1) {
              ...
              const normalizedExistingVolumeName =
                typeof (existing as any).volumeName === "string" &&
                (existing as any).volumeName.trim().length
                  ? (existing as any).volumeName.trim()
                  : undefined;
              const normalizedIncomingVolumeLocalChapter =
                Number.isFinite((dc as any).volumeLocalChapter) &&
                Number((dc as any).volumeLocalChapter) > 0
                  ? Number((dc as any).volumeLocalChapter)
                  : undefined;
              const normalizedExistingVolumeLocalChapter =
                Number.isFinite((existing as any).volumeLocalChapter) &&
                Number((existing as any).volumeLocalChapter) > 0
          ...
          const volumeOrder = buildVolumeOrderFromDriveSync(
            inventoryVolumeOrder ?? existingBook.settings?.volumeOrder,
      ...
      if (isCancelled()) return;
      // Re-read progressStore and apply to books so UI reflects Drive/sync progress
      const storeAfterSync = readProgressStore();
      const booksWithProgress = updatedBooks.map((book) => {
        const progressMap = storeAfterSync.books?.[book.id] || {};
        const chaptersWithProgress = (book.chapters || []).map((c) => {
          const p = progressMap[c.id];
          if (!p) return c;
          const progressChars =
      ...
      setState((p) => {
        ...
        const mergedBooks = booksWithProgress.map((book) => {
      ...
      pushNotice({
        message: "Sync Failed: " + (e?.message || "An error occurred"),
        type: "error",
        ms: 0,
      });
      return;
    ...
    try {
      ...
      if (restoreResult.snapshot.chapterTombstones?.length) {
        for (const t of restoreResult.snapshot.chapterTombstones) {
          try {
            await libraryUpsertChapterTombstone(t.bookId, t.chapterId, t.deletedAt);
          } catch (e) {
            console.warn("[Sync] tombstone restore failed", t.chapterId, e);
          }
        }
      }
  ...
  const handleSync = async (manual = false) => {
    if (manual) {
      ...
      pushNotice({ message: "Sync started", type: "info", ms: 1500 });
    }
    await performFullDriveSync(manual);
    await handleSaveState(true, false);
  };
  ...
  const handleLaunchSignIn = useCallback(async () => {
    ...
    try {
      await ensureAuthReady(state.googleClientId);
      await ensureValidToken(true);
      await runStartup();
    } catch (e: any) {
  ...
  }, [runStartup, state.googleClientId]);
  ...
  const handleReaderBack = useCallback(() => {
    ...
    const target =
      navContextRef.current?.lastNonReaderViewType ?? lastNonReaderTabRef.current ?? fallback;
        ...
        const next =
          typeof cached === "number" && Number.isFinite(cached)
            ...
            : navContextRef.current?.bookId === bookId &&
              ...
              ? navContextRef.current.collectionScrollTop
              : 0;
        collectionScrollByBookRef.current[bookId] = next;
        setCollectionRestoreScrollTop((prev) => (prev === next ? prev : next));
      }
    }
    setActiveTab(target);
  }, []);
  ...
  // Phase 4.2: Native – persist last active book id for cold-start restore (Preferences survives when localStorage may not).
  useEffect(() => {
    if (typeof window === "undefined" || !Capacitor.isNativePlatform()) return;
    void Preferences.set({ key: LAST_ACTIVE_BOOK_ID_KEY, value: state.activeBookId ?? "" }).catch(
      () => {}
    );
  }, [state.activeBookId]);
  ...
  const LinkCloudModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
        <div
          className={`w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 space-y-6 animate-in zoom-in-95 ${state.theme === Theme.DARK ? "bg-slate-900 border border-white/10" : "bg-white"}`}
        >
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-black tracking-tight flex items-center gap-3">
              <FolderSync className="w-6 h-6 text-indigo-600" /> Link Cloud Folder
            </h3>
            <button
          ...
          <div className="space-y-4">
            <p className="text-xs font-bold opacity-60 leading-relaxed">
              Select an existing TaleVox folder or create a new one to sync your library.
            </p>
            ...
            <div className="max-h-[40vh] overflow-y-auto space-y-2 pr-2">
              {loading ? (
                ...
                <div className="py-8 text-center text-[10px] font-black uppercase opacity-30">
                ...
                candidates.map((c) => (
                  <button
                    ...
                    className={`w-full p-4 rounded-2xl border-2 text-left transition-all flex items-center justify-between ${state.theme === Theme.DARK ? "bg-white/5 border-white/5 hover:border-indigo-600" : "bg-black/5 border-transparent hover:border-indigo-600"}`}
                  >
                    <div className="min-w-0">
                      <div className="text-sm font-black truncate">{c.name}</div>
                      {c.hasState && (
                        <div className="text-[9px] font-black text-emerald-500 uppercase mt-0.5 flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" /> Contains TaleVox State
                        </div>
                      )}
                    </div>
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2"
            >
  ...
  const diagnosticsConfig =
    state.showDiagnostics
      ? {
          ...
          chapterSession: chapterSessionForDiag,
  ...
  if (launchStage === "signin") {
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${shellThemeClass}`}>
        <div
          className={`w-full max-w-md rounded-[2rem] shadow-2xl p-8 space-y-6 ${state.theme === Theme.DARK ? "bg-slate-900 border border-white/10" : "bg-white border border-black/5"}`}
        >
          <div className="text-2xl font-black tracking-tight">TaleVox</div>
          <p className="text-xs font-bold opacity-60 leading-relaxed">
            Sign in to sync your library across devices.
          </p>
          {signInError && <div className="text-xs font-bold text-red-400">{signInError}</div>}
          <button
  ...
  return (
    ...
    >
      {launchStage === "ready" && startupProgressConflict && (
        <div
          className="fixed inset-0 z-[120] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4"
          onClick={() => {
            if (!applyingStartupProgressChoice) {
              void resolveStartupProgressConflict("durable");
            }
          }}
        >
          <div
            className={`w-full max-w-lg rounded-3xl border p-6 space-y-4 ${
              state.theme === Theme.DARK
          ...
          >
            <div className="text-lg font-black tracking-tight">Progress mismatch detected</div>
            <p className="text-xs font-semibold opacity-75 leading-relaxed">
              We found a major difference between durable and local progress snapshots. Choose which
              source to keep.
            </p>
            <div className="text-[11px] font-semibold opacity-70 space-y-1">
              <div>Detected differences: {startupProgressConflict.analysis.mismatchCount}</div>
              <div>Reasons: {startupProgressConflict.analysis.reasons.join(", ") || "unknown"}</div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              <button
                onClick={() => void resolveStartupProgressConflict("durable")}
                disabled={applyingStartupProgressChoice}
                className="py-3 rounded-2xl bg-indigo-600 text-white font-black uppercase text-[10px] tracking-widest disabled:opacity-60"
              >
                {applyingStartupProgressChoice ? "Applying..." : "Use durable"}
              </button>
              <button
                onClick={() => void resolveStartupProgressConflict("local")}
                disabled={applyingStartupProgressChoice}
                className="py-3 rounded-2xl border border-indigo-400 text-indigo-400 font-black uppercase text-[10px] tracking-widest disabled:opacity-60"
              >
                Use local
              </button>
            </div>
        ...
        <div className="absolute inset-0 z-[60] overflow-y-auto p-4 lg:p-12 backdrop-blur-md bg-black/10">
          <div className="max-w-4xl mx-auto relative">
            <button
              ...
              className="absolute -top-4 -right-4 p-3 bg-white text-black shadow-2xl rounded-full hover:scale-110 active:scale-95 transition-transform z-10"
            >
              <X className="w-6 h-6" />
            </button>
            <Suspense
              fallback={
                <div className="p-6 text-xs font-black uppercase tracking-widest opacity-60 flex items-center gap-2">
                  <Loader2 className="w-4 h-4 animate-spin" /> Loading import tools...
                </div>
              }
            >
              <LazyExtractor
        ...
        <div className="flex-1 min-w-0 min-h-0 flex flex-col overflow-hidden">
          <div className="flex-1 min-w-0 min-h-0 flex overflow-hidden">
            <aside className="hidden lg:block w-72 border-r border-black/5 bg-black/5 overflow-y-auto flex-shrink-0">
              <ChapterSidebar
                book={activeBook}
                theme={state.theme}
                onSelectChapter={handleSmartOpenChapter}
                onClose={() => {}}
                isDrawer={false}
            ...
            <div className="flex-1 min-w-0 min-h-0 overflow-hidden flex flex-col">
              {activeBook && activeChapterMetadata && (
                <Reader
                  chapter={activeChapterMetadata}
                  rules={[...state.globalRules, ...activeBook.rules]}
                  theme={state.theme}
                  chapterText={isLoadingChapter ? "" : activeChapterText}
                  speechText={activeSpeechText}
                  readerBlocks={isLoadingChapter ? [] : activeReaderModel.blocks}
                  contentLoading={isLoadingChapter}
                  ...
                  debugMode={state.debugMode}
                  onToggleDebug={() => setState((p) => ({ ...p, debugMode: !p.debugMode }))}
                  onJumpToOffset={handleJumpToOffset}
                  onBackToCollection={handleReaderBack}
          ...
          <div className="flex-shrink-0">
            <Player
              ...
              onNext={() => handleNextChapterRef.current(false)}
              onPrev={handlePrevChapter}
              onSeek={handleSeekByDelta}
              ...
              readerSettings={state.readerSettings}
              onUpdateReaderSettings={(patch) =>
                setState((p) => ({ ...p, readerSettings: { ...p.readerSettings, ...patch } }))
              }
              progressChars={state.currentOffsetChars}
              totalLengthChars={activeSpeechText.length}
              wordCount={activeChapterMetadata?.wordCount || 0}
              onSeekToOffset={handleJumpToOffset}
              sleepTimer={sleepTimerSeconds}
              onSetSleepTimer={setSleepTimerSeconds}
              stopAfterChapter={stopAfterChapter}
              onSetStopAfterChapter={setStopAfterChapter}
              useBookSettings={activeBook?.settings.useBookSettings || false}
              onSetUseBookSettings={(v) => {
                if (activeBook)
                  setState((p) => ({
                    ...p,
                    books: p.books.map((b) =>
                      b.id === activeBook.id
                        ? { ...b, settings: { ...b.settings, useBookSettings: v } }
                        : b
                    ),
                  }));
              }}
      ...
      {isChapterSidebarOpen && activeBook && (
        <div className="fixed inset-0 z-[60] flex">
          ...
          />
          <div
            className={`relative w-[85%] max-sm max-w-sm h-full shadow-2xl animate-in slide-in-from-left duration-300 ${state.theme === Theme.DARK ? "bg-slate-900" : state.theme === Theme.SEPIA ? "bg-[#efe6d5]" : "bg-white"}`}
          >
            <ChapterSidebar
              book={activeBook}
              theme={state.theme}
              onSelectChapter={(id) => {
                handleSmartOpenChapter(id);
          ...
          {activeTab === "library" && (
            <Library
              ...
              onSelectBook={(id) => {
                ...
                collectionScrollByBookRef.current[id] = 0;
                setCollectionRestoreScrollTop(0);
                updateNavContext({
                  bookId: id,
              ...
              <LazyChapterFolderView
                ...
                onUpdateBookSettings={(s) => {
                  ...
                  setState((p) => {
                    const next = {
                      ...p,
                      books: p.books.map((book) =>
                        book.id === activeBook.id ? updatedBook : book
                      ),
                    };
                    if (s.defaultVoiceId) {
                  ...
                  markDirty();
                  if (s.volumeOrder != null && updatedBook.driveFolderId && isAuthorized) {
                    void syncBookInventoryToDrive(updatedBook);
                  }
                }}
                ...
                lastSavedAt={state.lastSavedAt}
                restoreScrollTop={collectionRestoreScrollTop}
                restoreChapterId={null}
                restoreChapterIndex={null}
                onScrollPositionChange={handleCollectionScroll}
                onResolveChapterText={async (bookId: string, chapterId: string) =>
                  ensureChapterContentLoaded(bookId, chapterId, chapterSessionRef.current)
                }
                onQueueGenerateJob={async (chapterIds: string[], voiceId?: string) => {
                  ...
                  if (jobRunnerAvailable && notificationStatus && !notificationStatus.granted) {
                    pushNotice({
                      message: "Enable notifications to run background jobs.",
                      type: "error",
                    });
                    setActiveTab("settings");
                  ...
                  if (pendingChapterIds.length !== chapterIds.length) {
                    ...
                    pushNotice({
                      message: `Skipping ${skipped} already queued chapter(s).`,
                      type: "info",
                    });
                  }
                  ...
                  if (missingChapterIds.length) {
                    pushNotice({
                      message: `Skipped ${missingChapterIds.length} chapter(s) with missing text. Run Fix Integrity or re-import.`,
                      type: "error",
                    });
                  }
                  ...
                  const updateAudioStatusForPrepared = async (
                    nextStatusById: Map<string, AudioStatus | undefined>
                  ) => {
                  ...
                  try {
                    ...
                    const runPreflight = async () => {
                      ...
                      for (const id of preparedChapterIds) {
                        ...
                        const relPath =
                          chapterTextLocalPaths[id] || `${appConfig.paths.textDir}/${filename}`;
                        try {
                          const stat = await Filesystem.stat({
                            path: relPath,
                            directory: Directory.Data,
                          ...
                          const size =
                            typeof (stat as any)?.size === "number" ? (stat as any).size : 0;
                    ...
                    if (preflight.missingFiles.length) {
                      ...
                      for (const id of preflight.missingFiles) {
                        ...
                        const relPath =
                          chapterTextLocalPaths[id] ||
                          ch.localPath ||
                          `${appConfig.paths.textDir}/${filename}`;
                        try {
                          await Filesystem.writeFile({
                            path: relPath,
                            directory: Directory.Data,
                          ...
                          jobLog.warn("preflight.writeFailed", {
                            id,
                    ...
                    const isLargeBookJob =
                      preparedChapterIds.length >=
                        (appConfig.cloud?.audioCloudBatchMinChapters ?? 50) ||
                      preparedChapterIds.length >= (activeBook.chapters?.length ?? 0) * 0.8;
                    const hasCloudBatchEndpoint =
                    ...
                    const useCloudBatch =
                      (state.readerSettings.useCloudBatchForAudio && hasCloudBatchEndpoint) ||
                      (isLargeBookJob && hasCloudBatchEndpoint);

                    if (useCloudBatch) {
                      await enqueueCloudGenerateBookAudio(payload, state.readerSettings.uiMode, {
                        userId: getAuthSessionInfo().userEmail ?? "anonymous",
                        driveRootFolderId: state.driveRootFolderId,
                      });
                    } else if (isLargeBookJob) {
                      await startBookGenerationJob(payload, state.readerSettings.uiMode);
                    } else {
                    ...
                    const notifDenied =
                      msg.includes("notifications_not_granted") ||
                      causeMsg.includes("notifications_not_granted");
                    if (notifDenied) {
                      pushNotice({
                        message: "Enable notifications to run background jobs.",
                        type: "error",
                    ...
                    books: prev.books.map((b) => {
                      ...
                      for (const incoming of newChapters) {
                        ...
                        const preferIncomingTitle =
                          ...
                          !incomingTitle.toLowerCase().startsWith("imported");
                        const incomingIndex =
                          typeof incoming.index === "number" && incoming.index > 0
                            ? incoming.index
                            : null;
                        const existingIndex =
                          typeof existing.index === "number" && existing.index > 0
                            ? existing.index
                            : null;
                        merged.set(incoming.id, {
                          ...existing,
              ...
              <LazyRuleManager
                ...
                onAddRule={(r) => {
                  if (r.global) {
                    ...
                    setState((p) => ({
                      ...p,
                      books: p.books.map((b) =>
                        b.id === activeBook.id ? { ...b, rules: [...b.rules, r] } : b
                      ),
                    }));
                  }
                  markDirty();
                ...
                onDeleteRule={(id, isGlobal) => {
                  if (isGlobal) {
                    setState((p) => ({
                      ...p,
                      globalRules: p.globalRules.filter((r) => r.id !== id),
                    }));
                  } else if (activeBook) {
                    setState((p) => ({
                      ...p,
                      books: p.books.map((b) =>
                        b.id === activeBook.id
                          ? { ...b, rules: b.rules.filter((r) => r.id !== id) }
                          : b
                      ),
                    }));
                  }
                ...
                onImportRules={(rules) => {
                  if (activeBook) {
                    setState((p) => ({
                      ...p,
                      books: p.books.map((b) =>
                        b.id === activeBook.id ? { ...b, rules: [...b.rules, ...rules] } : b
                      ),
                    }));
                    markDirty();
                  }
          ...
          {activeTab === "settings" && (
            ...
            >
              <LazySettings
                ...
                onUpdate={(s) =>
                  setState((p) => ({ ...p, readerSettings: { ...p.readerSettings, ...s } }))
                }
                theme={state.theme}
                ...
                isSyncing={isSyncing}
                googleClientId={state.googleClientId}
                onUpdateGoogleClientId={(id) => {
                  setState((p) => ({ ...p, googleClientId: id }));
                  authManager.init(id);
                }}
                onClearAuth={() => {
                  authManager.signOut();
                  setState((p) => ({ ...p, driveRootFolderId: undefined }));
                }}
                onSaveState={() => handleSaveState(true, false)}
                lastSavedAt={state.lastSavedAt}
            ...
            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
              <div className="space-y-2">
                ...
                {downloadedChapters.length === 0 ? (
                  <div className="text-sm font-black opacity-60 text-emerald-300">
                    No offline chapters
                  </div>
                ) : (
                  <div className="space-y-3">
                    {downloadedChapters.map((chapter) => (
                      <div
                        key={chapter.id}
                        className="border border-emerald-500/30 rounded-2xl p-4 bg-emerald-500/5 flex flex-col gap-1"
                      >
                        ...
                        <div className="flex items-center justify-between text-sm font-semibold">
                          ...
                          <span className="text-[10px] uppercase opacity-60">
                            {chapter.hasDriveAudio ? "Cloud copy present" : "Offline only"}
                          </span>
                        </div>
                        <div className="text-[12px] font-semibold">
                          {chapter.title || "Untitled chapter"}
                        </div>
                        <div className="text-[10px] opacity-60 break-words">
                          {chapter.localPath}
                        </div>
                      </div>
                    ))}
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-3 max-h-[40vh] overflow-y-auto">
                  {uploadQueueItems.length === 0 ? (
                    <div className="text-sm font-black opacity-60 text-indigo-300">
                      No pending uploads
                    </div>
                  ) : (
                    uploadQueueItems.map((item) => (
                      <div
                        key={item.id}
                        className="border border-white/10 rounded-2xl p-4 bg-slate-900/70 flex flex-col gap-2"
                      >
      ...
      {showAttachments && (
        <div className="fixed inset-0 z-[130] overflow-y-auto bg-black/60 backdrop-blur-sm">
          <div className="min-h-full flex items-start justify-center p-4 py-10">
            <div className="w-full max-w-md bg-[#0e1118] text-white rounded-[2rem] shadow-2xl overflow-hidden">
              {/* Header */}
              <div className="px-5 pt-5 pb-4 border-b border-white/8">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-indigo-600/25 flex items-center justify-center flex-shrink-0">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-white/35 font-bold uppercase tracking-widest mt-0.5">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={handleAddBookAttachment}
                      className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white text-[11px] font-black uppercase tracking-widest rounded-xl transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/8 hover:bg-white/15 transition-colors"
                    >
              ...
              {attachmentsList.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 gap-3 px-6 text-center">
                  <div className="w-16 h-16 rounded-3xl bg-white/5 flex items-center justify-center mb-1">
                    <Paperclip className="w-7 h-7 text-white/20" />
                  </div>
                  <div className="text-sm font-black uppercase tracking-widest text-white/30">
                    No attachments yet
                  </div>
                  <div className="text-[11px] text-white/20 font-bold">
                    Add PDFs, images, or notes
                  </div>
                </div>
              ) : (
                <div className="p-3 space-y-1.5 max-h-[62vh] overflow-y-auto">
                  {attachmentsList.map((att) => {
                    ...
                    const isEditing = attachmentEditingId === att.id;
                    return (
                      <div
                        key={att.id}
                        className="flex items-center gap-3 px-3 py-3 rounded-2xl bg-white/5 hover:bg-white/8 transition-colors"
                      >
                        {/* Icon */}
                        <div
                        ...
                        >
                          {isPdf ? (
                            <FileText
                              className={`w-5 h-5 ${isPdf ? "text-red-400" : "text-slate-400"}`}
                            />
                          ) : (
                            <File
                              className={`w-5 h-5 ${isImage ? "text-emerald-400" : "text-slate-400"}`}
                            />
                          )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          {isEditing ? (
                            <input
                              autoFocus
                              value={attachmentEditingFilename}
                              onChange={(e) => setAttachmentEditingFilename(e.target.value)}
                              onKeyDown={(e) => {
                                if (e.key === "Enter")
                                  commitRenameAttachment(att, attachmentEditingFilename);
                                if (e.key === "Escape") cancelEditingAttachment();
                              }}
                              className="w-full bg-white/10 text-white text-sm font-bold rounded-lg px-2 py-1 outline-none border border-indigo-500/60 focus:border-indigo-400 leading-snug"
                            />
                          ) : (
                          ...
                          {!isEditing && (
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[9px] uppercase font-bold text-white/35">
                          ...
                          {isEditing ? (
                            <>
                              <button
                                ...
                                className="w-9 h-9 flex items-center justify-center rounded-xl bg-indigo-600/30 hover:bg-indigo-600/50 text-indigo-300 transition-colors"
                                title="Save name"
                              >
                              ...
                              <button
                                ...
                                className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/15 text-white/50 hover:text-white/80 transition-colors"
                                title="Cancel"
                              >
                                <X className="w-4 h-4" />
                              </button>
                            ...
                            <>
                              {localReady ? (
                                <button
                                  ...
                                  className="px-3 py-1.5 bg-indigo-600/25 hover:bg-indigo-600/45 text-indigo-300 text-[10px] font-black uppercase tracking-widest rounded-xl transition-colors"
                                >
                                  Open
                                </button>
                              ) : (
                                <button
                                  ...
                                  className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-colors ${
                                    isDownloading
                                      ...
                                      : "bg-sky-600/25 hover:bg-sky-600/45 text-sky-300"
                                  }`}
                                >
                              ...
                              <button
                                ...
                                className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 hover:bg-indigo-600/25 text-white/50 hover:text-indigo-300 transition-colors"
                                title="Rename attachment"
                              >
                                <Pencil className="w-4 h-4" />
                              </button>
                              <button
                                onClick={() => handleDeleteBookAttachment(att)}
                                className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 hover:bg-red-600/25 text-white/50 hover:text-red-300 transition-colors"
          ...
          <div className="w-full max-w-4xl bg-slate-950 text-white rounded-3xl p-4 space-y-3 shadow-2xl">
            <div className="flex items-center justify-between">
              <div className="text-sm font-black uppercase tracking-widest truncate">
                {attachmentViewer.attachment.filename}
              </div>
              <button
            ...
            <div className="w-full h-[70vh] bg-black rounded-2xl overflow-hidden">
              {attachmentViewer.attachment.mimeType?.startsWith("image/") ? (
                <img
                  src={attachmentViewer.uri}
                  alt={attachmentViewer.attachment.filename}
