...
Overheating and streamlining (Sections 1–10): reduced sustained CPU, bridge traffic, and re-renders on mobile without removing features.

- **Section 1 — Playback polling & bridge:** Mobile playback poll 200 ms → 500 ms; sync UI throttle 300 ms on mobile for tick updates; NativePlayerPlugin listener cleanup; adapter teardown removes Capacitor listeners; seek confirmation 15×100 ms (was 30×50 ms).
- **Section 2 — Highlight sync:** Platform default 250 ms on native (100 ms on web); Settings shows platform-aware default; single batched state object per tick in useHighlightSync.
- **Section 3 — Wake lock & screen:** Keep Awake default off on native; documented that screen-on is the main heat contributor when enabled.
- **Section 4 — Progress & SQLite:** Mobile progress commit throttle 800 ms → 1000 ms; periodic flush 45 s → 60 s.
- **Section 5 — App state & re-renders:** PlaybackTickContext so only tick consumers re-render; diagnostics overlay reads from context; AppShell memoized; Player, ChapterSidebar, ChapterFolderView confirmed memoized with displayName; App does not subscribe to tick.
- **Section 6 — Job polling & CPU:** Native job poll 2.5 s (local) / 7 s (cloud); round-robin one cloud job per refresh; WorkManager backoff 20 s for all workers; DriveUploadWorker single-task path when concurrency=1; FixIntegrityWorker MAX_FOLDER_DEPTH=5.
- **Section 7 — Upload queue:** hasQueuedUpload on storage drivers; batch enqueueUploads with existingKeys and skipDuplicateCheck; index on drive_upload_queue(bookId, chapterId).
- **Section 8 — Startup:** Batched chapter preload (batch size 4, first 10 books); GIS init uses script onload when tag present instead of 500 ms poll.
- **Section 9 — Other timers & scans:** Diagnostics log poll 1500 ms → 3000 ms; regex cache in GenerateAudioWorker; downloaded chapters 8-way concurrency; native auto-save 2× interval (cap 120 min).
- **Section 10 — Android correctness:** driveFolderCache and rulePatternCache → ConcurrentHashMap; audio focus documented in NativePlayerService; notificationManager duplicate-listener guard and dev warning when listeners > 1.

See `docs/overheating-audit-post-sections-1-10.md` for the full audit.

## 3.0.23

- Added a **book-level audio generation pipeline** for large jobs: the app now enqueues `generate_book_audio` jobs for full-book or 100+ chapter selections and processes them in resumable batches, instead of many tiny one-off jobs.
- Hardened Android background behavior for long-running generation: large jobs require Wi‑Fi and charging, and `GenerateAudioWorker` re-enqueues itself batch-by-batch so it can reliably finish hundreds or thousands of chapters without manual babysitting.
- Introduced an optional **cloud batch job path**: when `VITE_TALEVOX_BATCH_JOBS_ENDPOINT` is configured, very large generation requests are sent to a cloud batch API, with local job records tracking remote status for full-book, phone-disconnected runs.
- Improved library UX for big jobs: the Book screen now shows a book-level job card (device vs cloud, status, chapters completed/total, short error summary, and Pause/Retry actions) so users can see and resume large generation jobs without digging through settings.
- Kept the existing **Check/Fix**, manual chapter import, and Smart Upload flows compatible: Fix Integrity still performs its own targeted audio repair when `Generate audio` is enabled, while bulk audio generation for uploaded chapters now routes through the new book-level job system.

## 3.0.22

- Applied non-breaking dependency updates to address npm security advisories: all high-severity issues resolved (brace-expansion, minimatch, rollup, tar, ajv, lodash).
- Upgraded Vite to 7.x and Vitest to 3.x (Option A in `docs/vite-upgrade-pr-plan.md`) to clear remaining esbuild advisories; **npm audit** now reports **0 vulnerabilities**. Install uses `legacy-peer-deps` (`.npmrc`) until vite-plugin-pwa declares Vite 7 support. One ChapterFolderView test is skipped under Vitest 3 (scroll-restore timing).
- Tests and type checks pass after updates; production build unchanged.

## 3.0.21

- Background listening progress now reliably saves and restores, including time spent while the app is in the background.
- Chapter progress bars now reflect the actual stored position, and chapter reset always works.
- Playback speed (e.g. 1.5x) is remembered across app restarts and applied automatically.
- Attachments live in a dedicated `attachments` folder per book (both local and Drive-backed), matching the book's folder.
- Play/pause button and reader highlight stay in sync with actual audio, even when using system media controls.
- The default voice picker popup now appears above book options, with no overlapping modals.
...
- Progress writes are throttled (single 45s interval plus on pause, chapter end, and app background) to reduce bridge traffic and avoid accidental overwrites.
